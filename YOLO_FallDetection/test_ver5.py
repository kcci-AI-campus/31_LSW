...
실행: python test_ver4.py --source 0
영상: python test_ver4.py --source sample.avi --headless --save-preview
    ...
    def update(self, t, obs):
        ...
        if self.state == 'falling_candidate':
            ...
            if upright:
                ...
                self.recover_since = None
        # [추가] 하강 순간을 못 봤어도(낮은 fps, 추적 끊김, 이미 쓰러진 뒤 등장 등)
        # 누움이 LYING_CONFIRM_SEC 이상 이어지면 낙상으로 확정한다.
        if (self.state != 'fallen' and self.lying_run_since is not None
